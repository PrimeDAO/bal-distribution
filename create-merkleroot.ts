...
const TRANCHES = {
  unclaimed: getTranche(
    ["0x1bf4e7D549fD7Bf9c6BA3Be8BD2b9Af62F086220","810000000000000000"],
    ["0x6d327055D5791a93545432906aFa229accf0B762","6030000000000000000"],
    ["0x137dcE536912b0313D534aE0d10d122eA4C7a9ad","0"],
    ["0xC4d9d1a93068d311Ab18E988244123430eB4F1CD","1170000000000000000"],
    ["0xa3eB2115D947C29882c9Ee2d8D7BEF98d0cA16FD","0"],
    ["0x47b72adb8cf42043E4796C2FEcF27E836E0341Fb","0"],
    ["0x0ecfd7C7b08F05d9d28b80AE3139E42817f73248","810000000000000000"],
...
async function writeProofsIPFS() {
  let i;
  let forIPFS = [];
  ...
  for(i=0;i<allocation.length;i++){
    ...
    let obj = {
      ...
      claim: allocation[i][1],
      proof: proof
    }
    forIPFS.push(obj)
  }
  fs.writeFile('./user-claims.json', JSON.stringify(forIPFS), (err) => {
      if (err) throw err;
  });
